...
def predict():
    if (request.method == 'POST'):
        ...
        improved_recommendations = pd.merge(recommendations, pickled_reviews_data[['prod_id', 'reviews_clean']],
                                            left_on='prod_id', right_on='prod_id', how='left')
        ...
        improved_recommendations_final['Positive_sentiment_rate'] = improved_recommendations_final[
            'Total_predicted_positive_reviews'].div(improved_recommendations_final['Total_reviews']).replace(np.inf, 0)

        improved_recommendations_final = improved_recommendations_final.sort_values(by=['Positive_sentiment_rate'],
                                                                                    ascending=False)
        improved_recommendations_final = pd.merge(improved_recommendations_final, pickled_mapping, left_on='prod_id',
